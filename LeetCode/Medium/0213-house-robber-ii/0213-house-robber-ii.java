class Solution {
    public int rob(int[] nums) {
        ...
        dp2[1] = nums[1]; // Initialize: if starting from the 2nd house, max at position 1 is nums[1]

        // Fill dp arrays
        for(int i=2; i<n; i++){
            dp1[i] = Math.max(dp1[i-1], dp1[i-2] + nums[i]); // Exclude last house: rob 0 to n-2
            dp2[i] = Math.max(nums[i] + dp2[i-2], dp2[i-1]); // Exclude first house: rob 1 to n-1
        }
